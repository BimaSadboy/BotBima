...
const { neg } = require('./lib/kodenegara')
...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:Bima & BAGAS\n' //& Nama OWNER! 
            + 'ORG: Bima & Bagas;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=6283862496089:+62 82338052276\n'  //NOMOR OWNER! 
            + 'END:VCARD'
prefix = '.'
blocked = []   
limitawal = 99999
...
const client = new WAConnection()
   ...
   console.log(color('[','white'),color('∆','red'),color(']','white'),color('QR code is ready, Scan now..','white'),color('ItsBima & Bagasツ','red'),color('X','lime'),color('LoL-Human','red'))
})
	...
	client.on('message-new', async (mek) => {
		...
		try {
			...
			const groupAdmins = isGroup ? getGroupAdmins(groupMembers) : ''
           	const ftoko = { key: { fromMe: false, participant: `6281241405996@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "productMessage": { "product": { "productImage":{ "mimetype": "image/jpeg", "jpegThumbnail": fs.readFileSync('./src/logobot.jpg') }, "title": `Subrek FotzYT & THE LION`, "productImageCount": 9999 }, "businessOwnerJid": `6283862496089@s.whatsapp.net`}}}
...
case 'addbadword':
		...
		case 'grouplist':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					client.updatePresence(from, Presence.composing) 
					teks = `\`\`\`Ini adalah list group Bima & Bagas :\n\n\`\`\``
					no = 0
...
「 INFO BOT 」
◨ Nama: Bima & Bagas
◨ Wa: wa.me/6283862496089
◨ Wa: wa.me/6282338052276
◨ Prefix: 「  ${prefix}  」
...
break
case 'ownermenu':
if (isBanned) return reply('Mampos Kamu sudah Terbenned. Enak Kan, Enak Lah!!')
wew = fs.readFileSync(`./src/logobot.jpg`)
lzain = `
╭──❲ OWNER MENU ❳
│❀${prefix}bc
...
break
case 'randommenu':
if (isBanned) return reply('Mampos kamu sudah terbenned, Satu Kata Buat Lu. Anjing Lu!')
wew = fs.readFileSync(`./src/logobot.jpg`)
lzain = `
╭──❲ RANDOM MENU ❳
│❀${prefix}waifu
...
break
case 'othermenu':
if (isBanned) return reply('Mampos kamu sudah terbenned!')
wew = fs.readFileSync(`./src/logobot.jpg`)
lzain = `
╭──❲ OTHER MENU ❳
│❀${prefix}tts
...
break
		...
		case 'lpr':
					...
					client.sendMessage('6283862496089@s.whatsapp.net', options, text, {quoted: mek})
					reply('Masalah telah di laporkan ke owner Bima Laporan palsu atau main² tidak akan ditanggapi.')
					break
					case 'antidelete':
		...
		case 'request':
					...
					client.sendMessage('6283862496089@s.whatsapp.net', options, text, {quoted: mek})
					client.sendMessage('6282338052276@s.whatsapp.net', options, text, {quoted: mek})
					client.sendMessage('6283862496089@s.whatsapp.net', options, text, {quoted: mek})
					reply('REQUEST ANDA TELAH SAMPAI KE OWNER BIMA & BAGAS REQUESTS PALSU ATAU MAIN² TIDAK AKAN DITANGGAPI.')
					break
		case 'blocklist': 
...
if (args.length == 0) return reply(`Idnya mana kak?`)
                ...
                case 'hidetag':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Mampos kamu sudah terbenned. Sono Coli Hidup Mu Penuh Gabut!')
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
                ...
                case 'afk':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Mampos kamu sudah terbenned . Gara Gara Lu Jelek Sih!')
                                tels = body.slice(4)
...
case 'add':
			...
			if (!isRegistered) return reply(ind.noregis())
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								console.log('Subrek THE LION , FotzYT')
								buffer = fs.readFileSync(ran)
		...
		case 'blockk':
				 ...
				 client.chatRead (from)
					if (!isGroup) return reply(ind.groupo())
				if (!isAdmin) return reply('*Only Owner bot*')
		...
		case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `*「 BROADCAST Bima & Bagas 」*\n\n${body.slice(4)}`)
						}
						reply('*Suksess broadcast* ')
					}
					break
...
break
                ...
                case 'thunder':
		if (!isRegistered) return reply(ind.noregis())
                    if (args.length == 0) return reply(`Example: ${prefix + command} Bima & Bagas`)
                    ini_txt = args.join(" ")
		...
		if (!isRegistered) return reply(ind.noregis())
                ...
                case 'freefire':
		if (!isRegistered) return reply(ind.noregis())
                    if (args.length == 0) return reply(`Example: ${prefix + command} Its Bima & Bagas`)
                    ini_txt = args.join(" ")
		...
		if (budy.includes(`6283862496089 & 6282338052276`)) {
                  reply(`Jangan Tag >:V`)
                  const Bima & Bagas = fs.readFileSync('./sticker/lord.webp');
                  client.sendMessage(from, Bima & Bagas, MessageType.sticker, {quoted: mek})
                  }
